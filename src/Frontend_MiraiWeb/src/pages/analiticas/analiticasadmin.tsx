...
const availableGraphs: GraphConfig[] = [
  {
    ...
    title: 'Estudiantes Registrados por Mes',
    description: 'Evolución mensual de nuevos estudiantes registrados en la plataforma',
    icon: IconUsers,
    color: '#4BCDF6',
  ...
  },
  {
    ...
    title: 'Tasa de Finalización del Quiz',
    description: 'Embudo que muestra cuántos usuarios inician vs completan el quiz vocacional',
    icon: IconTarget,
    color: '#FF369F',
    source: 'PostHog',
  ...
  },
  {
    ...
    title: 'Tiempo Promedio en Chat',
    description: 'Análisis del tiempo que los usuarios pasan en la pantalla del chat',
    icon: IconClock,
    color: '#FFAF21',
    source: 'PostHog',
  ...
  },
  {
    ...
    title: 'Tiempo Promedio en Pantallas',
    description: 'Tiempo que los usuarios pasan en cada pantalla de la aplicación',
    icon: IconActivity,
    color: '#9F8BEA',
    source: 'PostHog',
  ...
  },
  {
    ...
    title: 'Tiempo Promedio de Estudiantes Activos',
    description: 'Tiempo promedio que pasan en la app los estudiantes activos',
    icon: IconActivity,
    color: '#4BCDF6',
    source: 'PostHog',
...
export default function AnaliticasAdmin() {
  ...
  const getMonthlyData = () => {
    if (!analyticsData?.studentsJoinedByMonth) return [];

    const monthNames = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'];
    
    return monthNames.map((month, index) => ({
      month: month,
      estudiantes: analyticsData.studentsJoinedByMonth[String(index + 1)] || 0
    }));
  };
  ...
  const selectGraphFromSuggestion = async (graph: GraphConfig) => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/chat-admin/start', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        setChatSession({
          ...
          graphType: data.data.graphType,
          graphInfo: data.data.graphInfo,
          messages: [data.data.initialMessage]
        });
      ...
      setChatInputError(error.message || 'Error al conectar con el servidor');
    }
  };
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/chat-admin/message', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const analyzeWithAI = async (graphType: string, level: 'general' | 'profundo' = 'general') => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/analytics/analyze-graph', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f9f9f9' }}>
      {/* CONTENIDO PRINCIPAL - SIN MARGIN */}
      <div style={{ 
        flex: 1, 
        padding: '24px', 
        overflowY: 'auto',
        ...
        {/* Modal de Selección de Gráficas para Análisis */}
        <Modal
        ...
        >
          <Stack gap="md">
            <Text className="font-roboto text-gray-600">
              Selecciona cualquier gráfica para obtener un análisis inicial con IA. 
                    ...
                    <Group gap="md" align="flex-start">
                      ...
                      <div style={{ flex: 1 }}>
                        <Group justify="space-between" mb="xs">
                          ...
                          <Badge
                            variant="light"
                            color={graph.source === 'PostHog' ? 'violet' : 'blue'}
            ...
            <Card shadow="sm" padding="lg" radius="md" className="mb-6">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <Text className="font-bebas text-xl text-[#1D1A05] tracking-wide mb-1">
                    ANALÍTICAS EN TIEMPO REAL - ADMINISTRADOR
                  </Text>
                  <Text className="font-roboto text-sm text-gray-600">
              ...
              <div style={{ width: '100%', height: '400px', borderRadius: '8px', overflow: 'hidden' }}>
                <iframe
                  ...
                  style={{ border: 'none' }}
                  allowFullScreen
                  src="https://us.posthog.com/embedded/RxcB71bdrh-TDWSoV6WWJIIA6ZUhFA"
                  title="PostHog Analytics Dashboard - Admin"
                />
              </div>
            </Card>
          </Grid.Col>
            ...
            <Card shadow="sm" padding="lg" radius="md" className="h-full">
              <div className="mb-4 flex justify-between items-center">
                <div>
                  ...
                  <Text className="font-roboto text-sm text-gray-600">
                    Evolución mensual de registros en el año actual
                  </Text>
                </div>
                <Group gap="sm">
                  <Badge 
              ...
              <AreaChart
                ...
                dataKey="month"
                series={[
                  { name: 'estudiantes', color: '#4BCDF6', label: 'Estudiantes' }
                ]}
                curveType="monotone"
                gridAxis="xy"
                withGradient
                  ...
                  content: ({ label, payload }) => {
                    ...
                    return (
                      <div style={{
                        ...
                        <Text className="font-roboto text-sm font-semibold mb-1">
                          {label}
                        </Text>
                        <Text className="font-roboto text-sm" style={{ color: '#4BCDF6' }}>
                          Estudiantes: {payload[0].value}
                        </Text>
                      </div>
                    );
                  }
            ...
            {!selectedGraph ? (
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <Textarea
                    placeholder="@estudiantes, @quiz, @chat, @pantallas..."
      ...
      {/* Botón flotante para abrir chat - CON TOOLTIP */}
      {!chatSession && (
        <div style={{ position: 'relative' }}>
          {showChatTooltip && (
            <Paper
