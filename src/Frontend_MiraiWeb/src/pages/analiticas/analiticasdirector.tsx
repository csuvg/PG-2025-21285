...
export default function AnaliticasDirector() {
  const { authenticatedFetch } = useAuthenticatedFetch();

  const topCareersData = [
    { career: 'Licenciatura en Antropologia', estudiantes: 10 },
    { career: 'Administracion de Empresas', estudiantes: 7 },
    { career: 'Bioquimica y Microbiologia', estudiantes: 5 },
    { career: 'Licenciatura en Psicologia', estudiantes: 3 },
    { career: 'Ingeniería Química Industrial', estudiantes: 1 },
  ];

  const mockData = [
    { dimension: 'O', value: 2.3 },
    { dimension: 'C', value: 2.3 },
    { dimension: 'E', value: 4.3 },
    { dimension: 'A', value: 4.3 },
    { dimension: 'N', value: 5.3 },
  ];

  const dimensionDescriptions: Record<'O' | 'C' | 'E' | 'A' | 'N', string> = {
  ...
  // Configuración de gráficas analizables - CON ALIASES PARA @
  const availableGraphsDirector: GraphConfig[] = [
    {
      id: 'top-carreras-recomendadas',
      title: 'Top 5 Carreras más Recomendadas',
      description: 'Carreras con mayor demanda entre estudiantes',
      color: '#4BCDF6',
      icon: IconUsers,
    ...
    {
      id: 'bfi-dimension',
      title: 'Promedio por Dimensión BFI',
      description: 'Promedio de cada dimensión en el test BFI',
      color: '#9F8BEA',
      icon: IconChartBar,
      source: 'Base de Datos',
    ...
    {
      id: 'carreras-mas-visitadas',
      title: 'Carreras más Visitadas',
      description: 'Carreras con mayor cantidad de visitas en la app',
      color: '#FFAF21',
      icon: IconTarget,
      source: 'PostHog',
      gradient: 'linear-gradient(135deg, #FFAF21, #4BCDF6)'
    },
    {
      id: 'top-carreras-tiempo-lectura',
      title: 'Top 5 Carreras por Tiempo de Lectura',
      description: 'Carreras con mayor tiempo promedio de lectura en la app',
      color: '#4BCDF6',
      icon: IconSparkles,
      source: 'PostHog',
    ...
    {
      id: 'temas-interes-chat',
      title: 'Temas de Interés en Chat',
      description: 'Principales temas consultados por los estudiantes en el chat',
      color: '#9F8BEA',
      icon: IconMessage,
      source: 'PostHog',
      gradient: 'linear-gradient(135deg, #9F8BEA, #FFAF21)'
    }
  ];
  ...
  // Calcular carreras sugeridas (tests completados * 5)
  const suggestedCareers = analyticsData ? analyticsData.studentCompletedTests * 5 : 0;

  // Calcular tasa de éxito (porcentaje de estudiantes que completaron el test)
  ...
  const detectGraphMention = (text: string) => {
    ...
    if (match) {
      ...
      return availableGraphsDirector.filter(g => 
  ...
  const selectGraphFromSuggestion = async (graph: GraphConfig) => {
    setShowGraphSuggestions(false);
    
    try {
      const response = await fetch('http://localhost:4000/api/chat-director/start-graph-chat', {
        method: 'POST',
  ...
  const analyzeWithAI = async (graphType: string, level: 'general' | 'profundo' = 'general') => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/analytics-director/analyze-graph', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleGraphSelection = (graph: GraphConfig) => {
    setSelectedGraph(graph);
      ...
      element.innerHTML = `
        <div style="border-bottom: 2px solid #4BCDF6; padding-bottom: 20px; margin-bottom: 20px;">
          <h1 style="color: #4BCDF6; margin: 0 0 10px 0; font-size: 24px;">
            ANÁLISIS INTELIGENTE - ${selectedGraph.title}
          </h1>
        ...
        <div style="margin-bottom: 20px;">
          <h2 style="color: #4BCDF6; font-size: 16px; margin-top: 0;">💡 PUNTOS CLAVE</h2>
          <ul style="margin: 10px 0; padding-left: 20px;">
            ${aiAnalysis.puntosClave.map((punto: string) => `
        ...
        ${aiAnalysis.hallazgosImportantes.length > 0 ? `
          <div style="margin-bottom: 20px;">
            <h2 style="color: #FF369F; font-size: 16px; margin-top: 0;">🔍 HALLAZGOS IMPORTANTES</h2>
            ${aiAnalysis.hallazgosImportantes.map((hallazgo: any) => `
        ...
        <div style="margin-bottom: 20px;">
          <h2 style="color: #4BCDF6; font-size: 16px; margin-top: 0;">📊 ANÁLISIS DETALLADO</h2>
        ...
        <div style="margin-bottom: 20px;">
          <h2 style="color: #9F8BEA; font-size: 16px; margin-top: 0;">✅ RECOMENDACIONES ESTRATÉGICAS</h2>
          ${aiAnalysis.recomendaciones.map((rec: any) => `
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/chat-director/chat-with-graph', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f9f9f9' }}>
      ...
      <div style={{ 
        ...
        <Modal
          ...
          title={
            <Group gap="xs">
              <ThemeIcon size="lg" radius="xl" style={{ background: 'linear-gradient(135deg, #4BCDF6, #FF369F)' }}>
        ...
        >
          <Stack gap="md">
            <Text className="font-roboto text-gray-600">
              Selecciona una gráfica para obtener un análisis institucional con IA.
            </Text>
            <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
              {availableGraphsDirector.map((graph) => (
                <Card
                  key={graph.id}
                  shadow="sm"
                  padding="lg"
                ...
                >
                  <Group gap="md" align="flex-start">
                    <ThemeIcon size={50} radius="md" style={{ background: graph.gradient }}>
                      <graph.icon size={28} />
                    </ThemeIcon>
                    <div style={{ flex: 1 }}>
          ...
          {isAnalyzing ? (
            ...
            <Stack gap="lg">
              <Group justify="space-between">
                <Text className="font-roboto text-sm text-gray-600">
                  Este es un análisis general. Profundiza para obtener más detalles.
                </Text>
                <Button
              ...
              <Card shadow="sm" padding="lg" radius="md">
                ...
                >
                  {aiAnalysis.puntosClave.map((punto: string, index: number) => (
                    <List.Item key={index}>
              ...
              {aiAnalysis.hallazgosImportantes.length > 0 && (
                <Card shadow="sm" padding="lg" radius="md">
                  ...
                  <Stack gap="md">
                    {aiAnalysis.hallazgosImportantes.map((hallazgo: any, index: number) => (
              ...
              <Card shadow="sm" padding="lg" radius="md">
                ...
                <Stack gap="md">
                  {aiAnalysis.recomendaciones.map((rec: any, index: number) => (
                    <Card key={index} padding="md" radius="md" withBorder>
                      <Group justify="apart" mb="xs">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              >
                ANALÍTICAS DE DIRECTOR
              </Text>
              <Text 
                className="font-roboto text-gray-600 text-lg"
              >
                Panel de control para seguimiento institucional
              </Text>
            </div>
        ...
        <Grid>
          <Grid.Col span={12}>
            <Card shadow="sm" padding="lg" radius="md" className="mb-6">
              <Text className="font-bebas text-xl text-[#1D1A05] tracking-wide mb-1">
                ANALÍTICAS EN TIEMPO REAL - DIRECTOR
              </Text>
              <Text className="font-roboto text-sm text-gray-600 mb-4">
                Dashboard institucional con métricas de dirección
              </Text>
              <div style={{ width: '100%', height: '400px', borderRadius: '8px', overflow: 'hidden' }}>
                <iframe
                  ...
                  style={{ border: 'none' }}
                  allowFullScreen
                  src="https://us.posthog.com/embedded/UikhsUUSctFJattMTL6ONDhX_w1Ssw"
                  title="PostHog Analytics Dashboard - Director"
                />
              </div>
            </Card>
          </Grid.Col>
            ...
            <Card shadow="sm" padding="lg" radius="md" className="h-full">
              <div className="mb-4 flex justify-between items-center">
                <div>
                  ...
                  <Text className="font-roboto text-sm text-gray-600">
                    Carreras con mayor demanda entre estudiantes
                  </Text>
                </div>
          ...
          <Grid.Col span={12}>
            <Card shadow="sm" padding="lg" radius="md" className="h-full">
              <div className="mb-4">
                <Text className="font-bebas text-xl text-[#1D1A05] tracking-wide">
                  PROMEDIO POR DIMENSIÓN BFI
                </Text>
                <Text className="font-roboto text-sm text-gray-600">
                  Promedio de cada dimensión en el test BFI
                </Text>
              </div>
              <ResponsiveContainer width="100%" height={400}>
                <RadarChart data={mockData}>
                  ...
                  <PolarAngleAxis 
                    dataKey="dimension" 
                    tickFormatter={(dimension) => `${dimension}`}
                  />
                    ...
                    content={({ payload }) => {
                      ...
                      return (
                        <div style={{
                          ...
                          <Text className="font-roboto text-sm font-semibold">
                            {payload[0].payload.dimension}: {payload[0].value}
                          </Text>
                        </div>
                      );
                    }}
              ...
              <div className="mt-4">
                <Text className="font-roboto text-sm text-gray-600 mb-2">
            ...
            {!selectedGraph ? (
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <Textarea
                    placeholder="@carreras, @bfi, @visitadas, @lectura, @chat..."
                      ...
                      <div style={{ flex: 1 }}>
                        <Group gap="xs" mb={4}>
                           <Badge
                            size="sm"
                            variant="light"
                            color={selectedGraph.source === 'PostHog' ? 'violet' : 'blue'}
                          >
